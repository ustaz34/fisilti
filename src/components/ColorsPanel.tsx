...
import { previewActivationSound, previewDeactivationSound } from "../lib/soundEffects";
import {
  loadThemeConfig,
    ...
    <div className="cp-root">
      ...
      <div className="cp-section">
        <div className="cp-section-head">
          ...
          <span className="cp-section-title">Ses Efekti</span>
          <div className="cp-section-line" />
        </div>

        <div className="cp-option-grid">
          {SOUND_PACKS.map((pack) => {
            const isActive = pack.id === config.soundPack;
            return (
              <button
                key={pack.id}
                className={`cp-option-card ${isActive ? "cp-option-card--active" : ""}`}
                onClick={() => {
                  update({ soundPack: pack.id });
                  if (pack.id !== "sessiz") {
                    previewActivationSound(pack.id);
                    setTimeout(() => previewDeactivationSound(pack.id), 300);
                  }
                }}
              >
                <span className="cp-option-name">{pack.name}</span>
                <span className="cp-option-desc">{pack.desc}</span>
              </button>
            );
          })}
        </div>
